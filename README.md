...
└─────────────────────────────────────────────────────────────────────────────┘
flowchart LR
    subgraph Этап1["Этап 1: Вход в объект"]
        A1[Осмотр объекта] --> A2[Чек-лист осмотра]
        A2 --> A3[КП черновик]
    end
    
    subgraph Этап2["Этап 2: Планирование"]
        B1[Назначение прораба] --> B2[План работ]
        B2 --> B3[Задачи монтажникам]
    end
    
    subgraph Этап3["Этап 3: Исполнение"]
        C1[Выполнение работ] --> C2[Отчёты монтажников]
        C2 --> C3[Расчёт ЗП]
        C2 --> C4[Заявки на материалы]
    end
    
    subgraph Этап4["Этап 4: Сдача"]
        D1[Чек-лист приёмки] --> D2[Акт сдачи]
    end
    
    Этап1 --> Этап2 --> Этап3 --> Этап4
